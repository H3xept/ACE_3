...
#define k_FLAG_DEFAULT_CAPS "--default"

#define k_DEFAULT_PROGRAM_NAME "./VirtualMachine/vm_programs/binaryCounter.bin"
...
Program* handle_program_loading_flag(const char* flag, int* argc, char const *argv[])
{
	...
	if (!strcmp(flag,k_FLAG_DEFAULT) || !strcmp(flag,k_FLAG_DEFAULT_CAPS)) {
...
void setup_program_mode(Program_Mode mode, VirtualMachine* vm, Disassembler* disasm)
{
	switch(mode)
	{
...
int main(int argc, char const *argv[])
{
	...
	setup_program_mode(handle_running_mode_flag(argv[1]));
